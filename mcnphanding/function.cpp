...
int calSymmetryAxisX(McnpFillStruct mcnpgeo999)
{
	...
	for (int i = 0; i < mcnpgeo999.voxelcount; i++)
	{
		...
		{
			XYZtmp = convertSequenceToXYZ(i, mcnpgeo999);
			if (XYZtmp.z > (mcnpgeo999.dimzsup - mcnpgeo999.dimzinf) * 0.7)
			{
				Xsum += XYZtmp.x;
				counttmp++;
			}
...
void organCopy(unsigned int organfrom, unsigned int organfor , McnpFillStruct mcnpgeo999)
{
	...
	for (int i = 0; i < mcnpgeo999.voxelcount; i++)
	{
		if (organfor == *(mcnpgeo999.element + i))
			*(mcnpgeo999.element + i) = 119;
	}
	std::cout << "organ " << organfor << " clean" << std::endl;
	...
	for (int i = 0; i < mcnpgeo999.voxelcount; i++)
	{
		if (organfrom == *(mcnpgeo999.element + i))
		{
			...
			if (*(mcnpgeo999.element + convertXYZToSequence(XYZtmp.x, XYZtmp.y, XYZtmp.z, mcnpgeo999)) == 119 ||
				*(mcnpgeo999.element + convertXYZToSequence(XYZtmp.x, XYZtmp.y, XYZtmp.z, mcnpgeo999)) >= 106 && *(mcnpgeo999.element + convertXYZToSequence(XYZtmp.x, XYZtmp.y, XYZtmp.z, mcnpgeo999)) <= 109)
			{
				*(mcnpgeo999.element + convertXYZToSequence(XYZtmp.x, XYZtmp.y, XYZtmp.z, mcnpgeo999)) = organfor;
				replacecount++;
			}
...
void volAdjust(char* inputpath, McnpFillStruct mcnpgeo999)
{
	...
	// lences adjust
	std::cout << "\n lence deform" << std::endl;
	adjustlence(66, ref[66], mcnpgeo999);
	adjustlence(68, ref[68], mcnpgeo999);

	float real[200]; // store real vol
	for (int i = 0; i < 200; i++)
		real[i] = -1;
	for (int i = 0; i < mcnpgeo999.voxelcount; i++)
	{
		real[*(mcnpgeo999.element + i)] ++;
	}
	for (int i = 0; i < 200; i++)
	{
		if (real[i] != -1)
		{
			real[i] += 1;
			real[i] = real[i] * mcnpgeo999.voxelsizex * mcnpgeo999.voxelsizey * mcnpgeo999.voxelsizez;
		}
	}

	std::cout << "======================" << std::endl;

	//relative error >= 1%, print waring message
	for (int i = 0; i < 200; i++)
	{
		if (real[i] != -1)
		{
	...
	if (fabs(real[62] - real[64]) / real[64] >= 0.05)
	{
		std::cout << "Organ " << 62 << "didn't pass bilateral check" << std::endl;
	}
	if (fabs(real[63] - real[65]) / real[65] >= 0.05)
	{
		std::cout << "Organ " << 63 << "didn't pass bilateral check" << std::endl;
	}
	if (fabs(real[66] - real[68]) / real[68] >= 0.05)
	{
		std::cout << "Organ " << 66 << "didn't pass bilateral check" << std::endl;
	}
	if (fabs(real[67] - real[69]) / real[69] >= 0.05)
	{
		std::cout << "Organ " << 69 << "didn't pass bilateral check" << std::endl;
	}
	if (fabs(real[129] - real[130]) / real[129] >= 0.05)
	{
		std::cout << "Organ " << 129 << "didn't pass bilateral check" << std::endl;
	}
